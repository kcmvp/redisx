...
func start(indexes ...schema.Index) {
	var err error
	...
	if duplicated := lo.FindDuplicatesBy(indexes, func(idx schema.Index) string {
		return idx.Name()
	}); len(duplicated) > 0 {
...
func Start(address string, maxConn int, indexes ...schema.Index) {
